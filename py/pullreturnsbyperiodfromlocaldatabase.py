...
class perform:
    ...
    SourceName = property(get_SourceName, set_SourceName)
    ...
    def __init__(self
        ...
        print '--- about to run'
        self.ReturnsDataframe = self.getreturnsfromlocaldatabase(period=period,sourcename=sourcename)
        
        self.PriceHistoryDataframe = self.ReturnsDataframe
        if len(self.PriceHistoryDataframe) > 0:
            self.FirstDateOfPriceHistory = self.PriceHistoryDataframe.index.tolist()[0]
        
        #print self.ReturnsDataframe
        

    def getreturnsfromlocaldatabase(self,period='Monthly',sourcename='PSN'):
        symbol = self.Symbol
        import datetime
        today_date = datetime.date.today()
        ...
        cursor.execute("SELECT * from ProductValues where ProductName = '" + symbol + "' and Measure = 'Returns " + period + "' and SourceName = '" + sourcename + "' and Period >= '" + self.StartDateString[:7] + "' and Period <= '" + self.EndDateString[:7] + "' order by Period")
        
        print "SELECT * from ProductValues where ProductName = '" + symbol + "' and Measure = 'Returns " + period + "' and SourceName = '" + sourcename + "' and Period >= '" + self.StartDateString[:7] + "' and Period <= '" + self.EndDateString[:7] + "' order by Period"
