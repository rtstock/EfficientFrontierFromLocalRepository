...
#    DailyReturnsDataframe = property(get_DailyReturnsDataframe, set_DailyReturnsDataframe)
    ...
    def __init__(self
        ...
        if source == 'local':
            ...
            self.StockHistoryDataframe = pp.stockhistorybackfilledtodatframeofstockhistoryinstancesusingcache('aapl',startdate_string,my_enddate_string) #,str(today_date))
        else:
            
            if period == 'monthly':
                self.StockHistoryDataframe = pp.stockhistorybackfilledtodatframeofstockhistoryinstancesusingcache(symbol,startdate_string,my_enddate_string) #,str(today_date))
                #self.MonthDayCharacter = 'M'
            else:
                self.StockHistoryDataframe = pp.stockhistorynobackfilltodataframeusingcache(symbol,startdate_string,my_enddate_string) #,str(today_date))
        ...
        while 1:
            ...
            for r1 in row:
                ...
                if i==3:
                    ...
                    b_periodend = d3.date()
                if i==4:
                    e_pctchange = float(r1)
                    e_logreturn = float(r1)
                    d_end = 0.0
